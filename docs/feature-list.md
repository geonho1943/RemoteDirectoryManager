...
- 저장소 루트가 없으면 서버 시작 시 생성되며, 상대 설정 경로는 프로세스 작업 디렉터리를 기준으로 한다.
- 루트 경로는 `/` 로 표현된다.
...
- 동작: 보호된 API 호출 과정에서 `API-Key` 헤더를 보내면 서버가 SHA-256 해시 비교로 인증한다. 클라이언트 로그인은 `GET /api/v1/auth/verify` 로 파일 저장소 상태와 분리해 키를 검증한다.
